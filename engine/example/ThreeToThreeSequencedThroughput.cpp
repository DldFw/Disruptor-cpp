...
ThreeToThreeSequencedThroughput::ThreeToThreeSequencedThroughput()
{
	for (auto i = 0; i < m_numPublishers; ++i)
	{
		...
		m_valuePublishers[i] = [this](const std::shared_ptr< CountdownEvent >& countdownEvent,
...
std::int64_t ThreeToThreeSequencedThroughput::run(Stopwatch& stopwatch)
{
...
std::int32_t ThreeToThreeSequencedThroughput::requiredProcessorCount() const
{
	return 4;
}

void ThreeToThreeSequencedThroughput::valuePublisher(const std::shared_ptr< CountdownEvent >& countdownEvent,
		const std::shared_ptr< RingBuffer< std::vector< std::int64_t > > >& ringBuffer,
		std::int64_t iterations,
		std::int32_t arraySize)
{
