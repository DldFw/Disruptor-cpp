...
OneToThreeSequencedThroughput::OneToThreeSequencedThroughput()
{
...
std::int64_t OneToThreeSequencedThroughput::run(Stopwatch& stopwatch)
{
	...
	for (auto i = 0; i < m_numEventProcessors; ++i)
	{
		...
		PerfUtil::failIfNot(m_results[i], m_handlers[i]->value(),
...
std::int32_t OneToThreeSequencedThroughput::requiredProcessorCount() const
{
	return 4;
}
