...
TwoToTwoWorkProcessorThroughput::TwoToTwoWorkProcessorThroughput()
{
...
std::int64_t TwoToTwoWorkProcessorThroughput::run(Stopwatch& stopwatch)
{
...
std::int32_t TwoToTwoWorkProcessorThroughput::requiredProcessorCount() const
{
	return 4;
}


TwoToTwoWorkProcessorThroughput::ValuePublisher::ValuePublisher(const std::shared_ptr< CountdownEvent >& cyclicBarrier,  
...
void TwoToTwoWorkProcessorThroughput::ValuePublisher::run()
{
	m_cyclicBarrier->signal();
	m_cyclicBarrier->wait();
...
void TwoToTwoWorkProcessorThroughput::ValueAdditionWorkHandler::onEvent(ValueEvent& evt)
{
	auto value = evt.value;
	m_total += value;
}

std::int64_t TwoToTwoWorkProcessorThroughput::ValueAdditionWorkHandler::total() const
{
	return m_total;
}
