...
ThreeToOneSequencedThroughput::ThreeToOneSequencedThroughput()
{
	...
	{
		m_valuePublishers[i] = [this](const std::shared_ptr< s::CountdownEvent >& countdownEvent,
...
std::int64_t ThreeToOneSequencedThroughput::run(Stopwatch& stopwatch)
{
	m_scheduler->start(5);
...
std::int32_t ThreeToOneSequencedThroughput::requiredProcessorCount() const
{
	return 4;
}

void ThreeToOneSequencedThroughput::valuePublisher(const std::shared_ptr< s::CountdownEvent >& countdownEvent,
		const std::shared_ptr< RingBuffer< ValueEvent > >& ringBuffer,
		std::int64_t iterations)
{
	auto& rb = *ringBuffer;
