...
ThreeToOneSequencedBatchThroughput::ThreeToOneSequencedBatchThroughput()
{
...
std::int64_t ThreeToOneSequencedBatchThroughput::run(Stopwatch& stopwatch)
{
	m_cyclicBarrier->reset();

	auto latch = std::make_shared< s::ManualResetEvent >(false);
...
std::int32_t ThreeToOneSequencedBatchThroughput::requiredProcessorCount() const
{
	return 4;
}

ThreeToOneSequencedBatchThroughput::ValueBatchPublisher::ValueBatchPublisher(const std::shared_ptr< s::CountdownEvent >& cyclicBarrier,
...
void ThreeToOneSequencedBatchThroughput::ValueBatchPublisher::run()
{
