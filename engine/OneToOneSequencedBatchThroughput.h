 ...
 * 
 **/
class OneToOneSequencedBatchThroughput : public IThroughput
{
	public:
		OneToOneSequencedBatchThroughput();
	...
	private:
		...
		const std::int64_t m_expectedResult = accumulatedAddition(m_iterations) * m_batchSize;

		std::shared_ptr< Disruptor::RingBuffer< ValueEvent > > m_ringBuffer;
