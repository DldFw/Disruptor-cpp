...
class OneToOneRawBatchThroughput : public IThroughput
{
	...
	std::shared_ptr< Disruptor::ISequencer< boost::any > > m_sequencer = std::make_shared< Disruptor::SingleProducerSequencer< boost::any > >(m_bufferSize, std::make_shared< Disruptor::YieldingWaitStrategy >());
...
{
	public:
		explicit MyRunnable(const std::shared_ptr< Disruptor::ISequencer< boost::any > >& sequencer);

		void reset(const std::shared_ptr< ManualResetEvent >& latch, std::int64_t expectedCount);

		void run();

		std::shared_ptr< Disruptor::Sequence > sequence = std::make_shared< Disruptor::Sequence >(-1);

	private:
		std::shared_ptr< ManualResetEvent > m_latch;
		std::int64_t m_expectedCount;
		std::shared_ptr< Disruptor::ISequenceBarrier > m_barrier;
};
