...
bool ResetEvent::internalWait(Disruptor::ClockConfig::Duration timeDuration)
{
	std::unique_lock< decltype(m_mutex) > lock(m_mutex);
	...
	if (!m_signaled)
	{
		...
		if (timeDuration == Disruptor::ClockConfig::Duration())
		{
			do
			{
...
bool ResetEvent::wait(Disruptor::ClockConfig::Duration timeDuration)
{
